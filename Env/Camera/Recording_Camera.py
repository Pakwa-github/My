...
class Recording_Camera:
    def __init__(self, camera_position:np.ndarray=np.array([0.0, 6.0, 2.6]), 
                 ...
                 frequency=20, resolution=(640, 480), prim_path="/World/recording_camera"):
        ...
        # Attention: Remember to initialize camera before use in your main code. 
        # And Remember to initialize camera after reset the world!!

    def initialize(self, depth_enable:bool=False, pc_enable:bool=False, 
                   segment_prim_path_list=None):
    ...
    def get_rgb_graph(self, save_or_not:bool=False, 
                      save_path:str=get_unique_filename(
                          base_filename=f"./image",extension=".png")):
        '''
    ...
    def get_pointcloud_from_depth(self, show_original_pc_online:bool=False, show_downsample_pc_online:bool=False):
        ...
        # 对点云数据进行简单的过滤：仅保留 z 值大于 0.005 的点（去除噪声）
        mask = (point_cloud[:, 2] > 0.005)  
        point_cloud = point_cloud[mask]
        
        down_sampled_point_cloud = furthest_point_sampling(
            point_cloud, colors=None, n_samples=2048)
        if show_downsample_pc_online:
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(down_sampled_point_cloud)
            o3d.visualization.draw_geometries([pcd])
        # 与一个全 0 的颜色数组（扩展为 6 列，后 3 列为 0）拼接后返回
    ...
    def create_gif(self, save_path:str=get_unique_filename(
            base_filename=f"Assets/Replays/carry_garment/animation/animation",extension=".gif")):
        '''
    ...
    def get_point_cloud_data(self, save_or_not:bool=True, 
                             save_path:str=get_unique_filename(
                                 base_filename=f"./pc",extension=".pcd"), 
