    ...
    def train(self, rollout_data) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            if self.clip_range_vf is None:
                ...
                value_pred_clipped = old_values + th.clamp(
                    values - old_values, -clip_range_vf, clip_range_vf
                )
                value_losses = th.max(
                    F.mse_loss(returns, values),
                    F.mse_loss(returns, value_pred_clipped)
                )
...
class PointCloudExtractor(nn.Module):
    def __init__(self, observation_space, features_dim=256):
        ...
        # 全局特征提取
        self.global_conv = nn.Conv1d(512, 256, 1)
    ...
    def forward(self, observations):
        # observations: shape (batch_size, num_points, 3)
        # 检查输入是否有nan
        if torch.isnan(observations).any():
            print("Warning: input observations contain NaN")
        x = observations.transpose(1, 2)  # shape: (batch_size, 3, num_points)
...
class PointCloudPolicy(ActorCriticPolicy):
    def __init__(self, *args, **kwargs):
        # 使用 PointCloudExtractor 作为特征提取器
        super().__init__(
            *args,
            **kwargs,
            features_extractor_class=PointCloudExtractor,
            features_extractor_kwargs=dict(features_dim=256)
        )
        # 注意：初始化 log_std 参数，用于控制动作分布的标准差
        self.log_std = nn.Parameter(torch.zeros(3))  # 对于3维动作空间
        self.action_scale = 1.0
        self.action_bias = 0.0
    
    def _get_action_dist_from_latent(self, latent_pi):
        # 通过 action_net 得到动作均值（注意观察是否出现 nan）
        mean_actions = self.action_net(latent_pi)
        # 打印调试信息
        if torch.isnan(mean_actions).any():
            print("Warning: mean_actions contain NaN before tanh:", mean_actions)
        
        # 限制动作范围，常规 tanh 限定在[-1,1]，再缩放偏移
        mean_actions = torch.tanh(mean_actions) * self.action_scale + self.action_bias

        # 对 log_std 做范围限制，确保数值稳定（这里取值在[-20,2]）
        log_std = torch.clamp(self.log_std, -20, 2)
        
        # 若 mean_actions 依然存在 nan，则用 nan_to_num 替换（作为保护措施）
        mean_actions = torch.nan_to_num(mean_actions, nan=0.0)
        # 同时可打印最终的均值
        if torch.isnan(mean_actions).any():
            print("After nan_to_num, mean_actions still contain NaN:", mean_actions)
