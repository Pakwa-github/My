...
class RlEnvBase(gym.Env):
    ...
    def step(self, actions):
        """Apply the input actions to the task and step the simulation.

        Args:
            actions(Union[numpy.ndarray, torch.Tensor]): Buffer of action data.
        ...
        if self._world is None:
            print("Warning: World is not initialized, resetting environment...")
            return self.reset()
    ...
    def reset(self, seed=None, options=None):
        ...
        if self._world is None:
            ...
            self._world = World(backend=self._backend)
            self._world.reset()
            self._world.step(render=self._render)
